...
public class ListeRestosController implements Initializable {
    ...
    {   
        ...
        table_restos.setOnMouseClicked((MouseEvent event) -> {
        if (event.getClickCount() > 1) {
            try {
                Edition();
            } catch (IOException ex) {
                Logger.getLogger(ListeRestosController.class.getName()).log(Level.SEVERE, null, ex);
    ...
    {
        if (table_restos.getSelectionModel().getSelectedItem() != null) 
        {
    ...
    private void Edit(ActionEvent event) throws SQLException, IOException 
    {
        ...
        n=rd.nextInt(10000)+1;
            ...
            alert.setTitle("Restaurant modifié");
            alert.setHeaderText("Restaurant modifié avec succès");
            alert.setContentText("Restaurant "+nom1.getText()+" modifié avec succès");
            alert.showAndWait();
        }
        catch (Exception e)
                {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Restaurant non modifié");
                alert.setHeaderText("Echec de la modification du restaurant");
                alert.setContentText("Echec de la modification du restaurant "+nom1.getText());
                alert.showAndWait();
                }
        Close(event);
        }
    ...
    public void insertionBase(String nomFile)throws URISyntaxException, MalformedURLException, IOException 
            {
                ...
                urlb.addSubfolder("insertionImageResto.PHP");
                urlb.addParameter("photoresto","http://localhost/java/images/"+nomFile+".png");
                urlb.addParameter("nomresto",nom1.getText());
